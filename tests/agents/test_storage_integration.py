...
class TestDiagramGeneration:
    ...
    async def test_stores_generated_diagram(self, agent, mock_storage):
        ...
        with patch('diagram_generator.backend.agents.diagram_agent.ChatOllama') as mock_chat_cls:
            ...
            mock_chat.ainvoke.return_value = {"response": mock_code}
    ...
    async def test_stores_validation_and_fixes(self, agent, mock_storage):
        ...
        with patch('diagram_generator.backend.agents.diagram_agent.ChatOllama') as mock_chat_cls:
            ...
            mock_chat.ainvoke.side_effect = (
                [{"response": mock_codes[0]}] +  # Initial generation
                [{"response": validation_responses[0]}] +  # First validation (fails)
                [{"response": mock_codes[1]}] +  # Fix attempt
                [{"response": validation_responses[1]}]  # Second validation (succeeds)
            )
    ...
    async def test_stores_with_rag_context(self, agent, mock_storage):
        ...
        with patch('diagram_generator.backend.agents.diagram_agent.ChatOllama') as mock_chat_cls:
            mock_chat = AsyncMock()
            mock_chat_cls.return_value = mock_chat
            mock_chat.ainvoke.return_value = {"response": "graph TD;\nA-->B;"}
    ...
    async def test_stores_metadata(self, agent, mock_storage):
    ...
    async def test_stores_failing_attempts(self, agent, mock_storage):
        """Test storing failed generation attempts."""
        with patch('diagram_generator.backend.agents.diagram_agent.ChatOllama') as mock_chat_cls:
            mock_chat = AsyncMock()
            mock_chat_cls.return_value = mock_chat
            mock_chat.ainvoke.side_effect = Exception("Generation failed")
